...
const ParamsStorage = () => {
	useEffect(() => {
		function updateParamsObj(params_type) {
			...
			params["locations_dropdown1"] =
				sessionStorage["locations_dropdown1"];
			params["locations_dropdown2"] =
				sessionStorage["locations_dropdown2"];
			params["locations_dropdown3"] =
			...
			params["geo_country_selected"] =
				sessionStorage["geo_country_selected"];
			params["raw_string_query_input"] =
			...
			params["boolean_keywords_string"] =
				sessionStorage["boolean_keywords_string"];
			params["count_query"] = false;
			params["locations_dropdown"] = false;
			params["locations_dropdown_number"] = "0";
			params["fetch_people_data_count"] =
				sessionStorage["fetch_people_data_count"];
			params["fetch_companies_data_count"] =
				sessionStorage["fetch_companies_data_count"];
			params["fetch_nodes_data_count"] =
				sessionStorage["fetch_nodes_data_count"];
		...
		function updateStorageToken(token_obj, source) {
			console.log(
				"Updating Storage Token with token:: and source::",
				token_obj,
				source
			);
			console.log(
			...
			if (source == "viewTableinOrgChart") {
				...
				existing_tokens["company_id"] = [
					new UtilityFunctions().takeInputValueAndReturnObject(
			...
			if (source == "selected_function_dropdown") {
				...
				existing_tokens["std_function_category"] = [
					new UtilityFunctions().takeInputValueAndReturnObject(
						token_obj["std_function_category"]
					),
				];
			...
			if (source == "industry_category") {
				existing_tokens = JSON.parse(sessionStorage["query"]);
				existing_tokens["industry"] =
					new UtilityFunctions().turnArraytoArrayofObjects(
						token_obj["industry_category"]
					);
			...
			if (source == "gpt3_company_search") {
				...
				if (typeof token_obj["companies_ids_list"] != "undefined") {
					remove_duplicates_company_ids =
						new UtilityFunctions().getUniqueListBy(
							[].concat(
								existing_tokens["company_id"],
								new UtilityFunctions().turnArraytoArrayofObjects(
									token_obj["companies_ids_list"]
								)
							),
							"name"
						);
					remove_duplicates_company_names =
						new UtilityFunctions().getUniqueListBy(
							[].concat(
								existing_tokens["company_name"],
								new UtilityFunctions().turnArraytoArrayofObjects(
									token_obj["companies_names_list"]
								)
							),
							"name"
						);
			...
			if (source == "resetFunctionsGradesDropdown") {
				console.log(
					"Resetting functiona adn grades dropdown!!!-->resetFunctionsGradesDropdown"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				existing_tokens["std_function_category"] = [];
				existing_tokens["std_grade_category"] = [];
			...
			if (source == "resetOrgChartTokensAfterFetchNodeData") {
				console.log(
					"Got here to update storage Token wwith source:: resetSessionStorageTokensForContactInfo_tokens_update"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				existing_tokens["key"] = [];
				existing_tokens["std_function"] = [];
			...
			if (source == "selectFunctionsTreeFunctionsRootItem") {
				console.log(
					"Resetting functiona adn grades dropdown!!!-->selectFunctionsTreeFunctionsRootItem"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				existing_tokens["std_function_category"] = [
					new UtilityFunctions().takeInputValueAndReturnObject(
						token_obj["function_root"]
					),
				];
				existing_tokens["functions"] = [];
				sessionStorage["query"] = JSON.stringify(existing_tokens);
			...
			if (
				source ==
				"resetSessionStorageTokensForContactInfo_tokens_update"
			) {
				console.log(
					"Got here to update storage Token wwith source:: resetSessionStorageTokensForContactInfo_tokens_update"
				);
			...
			if (source == "viewOrgChartCompanyCard_token_obj") {
				console.log(
					"Got here to update storage Token wwith source:: viewOrgChartCompanyCard_token_obj"
				);
				...
				existing_tokens = JSON.parse(sessionStorage["query"]);
				if (
					company_name_selected != "undefined" ||
					company_name_selected != "null" ||
			...
			if (source == "close_token_object") {
				console.log(
					"Got here to update storage Token wwith source:: close_token_object"
				);
				...
				existing_query_obj_tokens = existing_query_obj_tokens.filter(
					(obj) => obj.name !== token_id_clicked
				);
			...
			if (source == "toggleOrgCharts") {
				console.log(
					"Got here to update storage Token wwith source:: toggleOrgCharts"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				existing_tokens["function_root"] = [];
			...
			if (source == "cost_of_credits_org_chart") {
				console.log(
					"Got here to update storage Token wwith source:: cost_of_credits_org_chart"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				existing_tokens["country"] = [];
			...
			if (source == "token_filter_background_minus") {
				console.log(
					"Got here to update storage Token wwith source:: token_filter_background_minus"
				);
				// token_obj = {"token_name":token_name, "dropdown_class_name":dropdown_class_name}
				existing_tokens = JSON.parse(sessionStorage["query"]);
				dropdown_class_name_tokens =
					existing_tokens[token_obj["dropdown_class_name"]];
				dropdown_class_name_tokens = dropdown_class_name_tokens.filter(
					(obj) => obj.name !== token_obj["token_name"]
				);
				updated_token_obj =
					new UtilityFunctions().takeInputValueAndReturnObject(
						token_obj["token_name"]
					);
			...
			if (source == "token_filter_background_plus") {
				console.log(
					"Got here to update storage Token wwith source:: token_filter_background_plus"
				);
				// token_obj = {"token_name":token_name, "dropdown_class_name":dropdown_class_name}
				existing_tokens = JSON.parse(sessionStorage["query"]);
				dropdown_class_name_tokens =
			...
			if (
				source ==
				"contextMenu_fetchCurrentPositionAcrossCompaniesResults"
			) {
				console.log(
					"Got here to update storage Token wwith source:: contextMenu_fetchCurrentPositionAcrossCompaniesResults"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				console.log(token_obj);
				console.log(existing_tokens);
				reverse_company_id_obj = JSON.parse(sessionStorage["query"])[
					"company_id"
				][0];
				existing_tokens["grades"].push(
					new UtilityFunctions().takeInputValueAndReturnObject(
						token_obj["std_grade"]
					)
				);
				existing_tokens["functions"].push(
					new UtilityFunctions().takeInputValueAndReturnObject(
						token_obj["std_function"]
					)
				);
			...
			if (source == "arxena_reversed") {
				console.log(
					"Got here to update storage Token wwith source:: arxena_reversed and token obj",
					token_obj
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
			...
			if (source == "functions_combo_tree") {
				console.log(
					"Got here to update storage Token wwith source:: functions_combo_tree"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
			...
			if (source == "checkbox_phone_number") {
				console.log(
					"Got here to update storage Token wwith source:: checkbox_phone_number"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
			...
			if (source == "checkbox_email_address") {
				console.log(
					"Got here to update storage Token wwith source:: checkbox_email_address"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
			...
			if (source == "filter_to_clear") {
				console.log(
					"Got here to update storage Token wwith source:: filter_to_clear"
				);
			...
			if (source == "raw_element_push") {
				console.log(
					"Got here to update storage Token wwith source:: raw_element_push"
				);
				console.log(
					"Got here to raw element push with token object::",
					token_obj
				);
				...
				existing_tokens = JSON.parse(sessionStorage["query"]);
				if (
					(element_focussed == "gpt3_company_search") &
					(input_value == "")
				) {
					existing_tokens[element_focussed] = [];
				}
				if (
					existing_tokens[element_focussed].filter(
						(e) => e.name === input_value
					).length > 0
				) {
					...
					remove_duplicates_existing_tokens =
						new UtilityFunctions().getUniqueListBy(
							[].concat(existing_tokens[element_focussed], [
								new UtilityFunctions().takeInputValueAndReturnObject(
									input_value
								),
							]),
			...
			if (source == "boolean_keywords_string") {
				node = token_obj;
				try {
					company_name = JSON.parse(
						sessionStorage["org_chart_data_company_name"]
					);
					company_id = JSON.parse(
						sessionStorage["org_chart_data_company_id"]
					);
					if (
						(company_name != "") &
						(typeof company_name != "undefined")
					) {
						updated_input_value_company_name =
							new UtilityFunctions().takeInputValueAndReturnObject(
								company_name
							);
						existing_tokens["company_name"] = [
					...
					if (
						...
						updated_input_value_company_id =
							new UtilityFunctions().takeInputValueAndReturnObject(
								company_id
							);
						existing_tokens["company_id"] = [
				...
				existing_tokens["key"] =
					node.data.key != null
						...
						: [];
				existing_tokens["std_function"] =
					node.data.std_function != null
						? [
								new UtilityFunctions().takeInputValueAndReturnObject(
									node.data.std_function
								),
						  ]
						: [];
				existing_tokens["functions"] =
				...
				existing_tokens["grades"] =
					node.data.std_grade != null
						...
						: [];
				existing_tokens["std_grade_category"] =
					node.data.std_grade_category != null
						? [
								new UtilityFunctions().takeInputValueAndReturnObject(
									node.data.std_grade_category
								),
						  ]
						: [];
				existing_tokens["std_function_category"] =
					node.data.std_function_category != null
						? [
								new UtilityFunctions().takeInputValueAndReturnObject(
						...
						: [];
				existing_tokens["function_root"] =
					node.data.function_root != null
						? [
								new UtilityFunctions().takeInputValueAndReturnObject(
									node.data.function_root
								),
						  ]
						: [];
			...
			if (source == "fetch_query_locations_dropdown") {
				console.log(
					"Got here to update storage Token wwith source:: fetch_query_locations dropdown"
				);
				...
				existing_tokens = JSON.parse(sessionStorage["query"]);
				// debugger;
				existing_tokens["key"] = node?.data?.key
					? [
							new UtilityFunctions().takeInputValueAndReturnObject(
								node.data.key
							),
					  ]
					: JSON.parse(sessionStorage["query"])["key"]
					? JSON.parse(sessionStorage["query"])["key"]
					: [];
				existing_tokens["std_function"] = node?.data?.std_function
					? [
					...
					: JSON.parse(sessionStorage["query"])["std_function"]
					? JSON.parse(sessionStorage["query"])["std_function"]
					: [];
				existing_tokens["functions"] = node?.data?.std_function
					? [
							new UtilityFunctions().takeInputValueAndReturnObject(
								node.data.std_function
							),
					  ]
					: JSON.parse(sessionStorage["query"])["functions"]
					? JSON.parse(sessionStorage["query"])["functions"]
					: [];
				existing_tokens["std_grade"] = node?.data?.std_grade
					? [
					...
					: JSON.parse(sessionStorage["query"])["std_grade"]
					? JSON.parse(sessionStorage["query"])["std_grade"]
					: [];
				existing_tokens["grades"] = node?.data?.std_grade
					? [
							new UtilityFunctions().takeInputValueAndReturnObject(
								node.data.std_grade
							),
					  ]
					: JSON.parse(sessionStorage["query"])["grades"]
					? JSON.parse(sessionStorage["query"])["grades"]
					: [];
				existing_tokens["std_grade_category"] = node?.data
					?.std_grade_category
					? [
							new UtilityFunctions().takeInputValueAndReturnObject(
								node.data.std_grade_category
					...
					: JSON.parse(sessionStorage["query"])["std_grade_category"]
					? JSON.parse(sessionStorage["query"])["std_grade_category"]
					: [];
				existing_tokens["std_function_category"] = node?.data
				...
				existing_tokens["function_root"] = node?.data?.function_root
					...
					: JSON.parse(sessionStorage["query"])["function_root"]
					? JSON.parse(sessionStorage["query"])["function_root"]
					: [];
				// existing_tokens['std_function'] = [node.data.std_function]
				// existing_tokens['std_grade'] = [node.data.std_grade]
			...
			if (source == "endpoint") {
				console.log(
					"Got here to update storage Token wwith source:: endpoint"
				);
				new ProcessNodesStates().resetSessionStorageforDropdowns();
				existing_tokens = JSON.parse(sessionStorage["query"]);
				if (
					(token_obj["updated_value"].length >= 1) &
					(token_obj["updated_value"] != " ") &
					(token_obj["updated_value"] != "") &
					(token_obj["updated_value"] != null)
				) {
					existing_tokens[token_obj["token_to_be_updated"]].push(
						new UtilityFunctions().takeInputValueAndReturnObject(
							token_obj["updated_value"]
						)
					);
			...
			if (source == "fetchCandidatesfromUploadJDCV") {
				console.log(
					"Got here to update storage Token wwith source:: fetchCandidatesfromUploadJDCV"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				existing_tokens["reverse_company"] =
					(token_obj["reverse_company"] != null) &
					(token_obj["reverse_company"] != "")
						? new UtilityFunctions().turnArraytoArrayofObjects(
								token_obj["reverse_company"]
						  )
						: [];
				existing_tokens["reverse_job_title"] =
					(token_obj["reverse_job_title"] != null) &
					(token_obj["reverse_job_title"] != "")
						? new UtilityFunctions().turnArraytoArrayofObjects(
								token_obj["reverse_job_title"]
						  )
						: [];
				existing_tokens["locations"] =
					token_obj["locations"] != null
						? new UtilityFunctions().turnArraytoArrayofObjects(
								token_obj["locations"]
						  )
						: [];
				existing_tokens["functions"] =
					token_obj["functions"] != null
						? new UtilityFunctions().turnArraytoArrayofObjects(
								token_obj["functions"]
						  )
						: [];
				existing_tokens["grades"] =
					token_obj["grades"] != null
						? new UtilityFunctions().turnArraytoArrayofObjects(
								token_obj["grades"]
						  )
						: [];
				existing_tokens["function_root"] =
					token_obj["function_root"] != null
						? new UtilityFunctions().turnArraytoArrayofObjects(
								token_obj["function_root"]
						  )
						: [];
				existing_tokens["company_name"] =
					(token_obj["company_name"] != null) &
					(typeof token_obj["company_name"] != "undefined") &
					(token_obj["company_name"] != "")
						? new UtilityFunctions().turnArraytoArrayofObjects(
								token_obj["company_name"]
						  )
						: [];
				existing_tokens["company_id"] =
					(token_obj["company_id"] != null) &
					(typeof token_obj["company_id"] != "undefined") &
					(token_obj["company_id"] != "")
						? new UtilityFunctions().turnArraytoArrayofObjects(
								token_obj["company_id"]
						  )
						: [];
			...
			if (token_obj == "locations_required1") {
				console.log(
					"Got here to update storage Token wwith source:: locations_required1"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				existing_tokens["country"] = [
					new UtilityFunctions().takeInputValueAndReturnObject(
			...
			if (token_obj == "locations_required2") {
				console.log(
					"Got here to update storage Token wwith source:: locations_required2"
				);
			...
			if (token_obj == "locations_required3") {
				console.log(
					"Got here to update storage Token wwith source:: locations_required3"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				existing_tokens["locality"] = [
					new UtilityFunctions().takeInputValueAndReturnObject(
			...
			console.log(
				"Just before ajax query:: sessionstoragequery is::",
				JSON.parse(sessionStorage["query"]),
				" and source is",
				source
			);
			if (source == "ajaxQuery") {
				console.log(
					"Got here to update storage Token wwith source:: ajaxQuery"
				);
				console.log(token_obj);
				...
				if (Array.isArray(token_obj)) {
					...
					if (
						existing_tokens["company_id"].filter(
							(e) => e.name === token_obj["company_id"]
						).length > 0
					) {
					} else if (
						(token_obj["company_id"] != "null") &
						...
						console.log(
							"Got in here to update the existing tokesn with company ID  in ajax query"
						);
						existing_tokens["company_id"].push(
							new UtilityFunctions().takeInputValueAndReturnObject(
								token_obj["company_id"]
							)
						);
					...
					if (
						existing_tokens["company_name"].filter(
							(e) => e.name === token_obj["company_name"]
						).length > 0
					) {
					} else if (
						(token_obj["company_name"] != "null") &
						...
						console.log(
							"Got in here to update the existing tokesn with company name  in ajax query"
						);
						existing_tokens["company_name"].push(
							new UtilityFunctions().takeInputValueAndReturnObject(
								token_obj["company_name"]
							)
						);
			...
			if (source == "ajaxQueryRemove") {
				console.log(
					"Got here to update storage Token wwith source:: ajaxQueryRemove"
				);
				console.log(token_obj);
				var existing_tokens = JSON.parse(sessionStorage["query"]);
				if (
					...
					existing_tokens["company_id"].filter(
						(obj) => obj.name !== token_obj["company_id"]
					);
				}
				if (
					existing_tokens["company_name"].filter(
					...
					existing_tokens["company_name"].filter(
						(obj) => obj.name !== token_obj["company_name"]
					);
				}
				sessionStorage["query"] = JSON.stringify(existing_tokens);
			}
			...
			if (source == "home_page_dropdown") {
				console.log(
					"Got here to update storage Token wwith source:: home_page_dropdown"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				if (
					(token_obj["company_id"] != "null") &
					(token_obj["company_id"] != "undefined")
				) {
					existing_tokens["company_id"] = [
						new UtilityFunctions().takeInputValueAndReturnObject(
							token_obj["company_id"]
				...
				if (
					(token_obj["company_name"] != "null") &
					(token_obj["company_name"] != "undefined")
				) {
					existing_tokens["company_name"] = [
						new UtilityFunctions().takeInputValueAndReturnObject(
							token_obj["company_name"]
				...
				if (
					(token_obj["website"] != "null") &
					(token_obj["website"] != "undefined")
				) {
					existing_tokens["website"] = [
						new UtilityFunctions().takeInputValueAndReturnObject(
							token_obj["website"]
						),
					];
			...
			if (source == "nodeSimilarPeople") {
				console.log(
					"Got here to update storage Token wwith source:: nodeSimilarPeople"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				// if (existing_tokens['functions'].includes(new UtilityFunctions().takeInputValueAndReturnObject(token_obj['std_function']))){}
				if (
					existing_tokens["functions"].filter(
						(e) => e.name === token_obj["std_function"]
					).length > 0
				) {
				...
				if (
					existing_tokens["grades"].filter(
						(e) => e.name === token_obj["std_grade"]
					).length > 0
				) {
				...
				if (
					existing_tokens["function_root"].filter(
						(e) => e.name === token_obj["function_root"]
					).length > 0
				) {
				} else if (token_obj["function_root"] != null) {
					existing_tokens["function_root"].push(
						new UtilityFunctions().takeInputValueAndReturnObject(
							token_obj["function_root"]
						)
					);
			...
			if (source == "selectGeoArea") {
				console.log(
					"Got here to update storage Token wwith source:: selectGeoArea"
				);
				console.log(token_obj);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				existing_tokens["country"] = [
			...
			if (source == "functional_area") {
				console.log(
					"Got here to update storage Token wwith source:: functional_area"
				);
				console.log(token_obj);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				existing_tokens["function_root"] = [
			...
			if (source == "contactInfoClicked") {
				console.log(
					"Got here to update storage Token wwith source:: contactInfoClicked"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				console.log(token_obj);
				console.log(existing_tokens);
				existing_tokens["person_id"] = [
					new UtilityFunctions().takeInputValueAndReturnObject(
						String(token_obj["person_id"])
					),
				];
				existing_tokens["person_linkedin_url"] = [
					new UtilityFunctions().takeInputValueAndReturnObject(
						String(token_obj["person_linkedin_url"])
					),
				];
				console.log(existing_tokens);
				console.log(JSON.stringify(existing_tokens));
			...
			if (source == "contextMenu") {
				console.log(
					"Got here to update storage Token wwith source:: contextMenu"
				);
				existing_tokens = JSON.parse(sessionStorage["query"]);
				console.log(token_obj);
				console.log(existing_tokens);
		...
		function addTokentoSessionStorage(user_input) {
			...
			company_id_clicked = user_input.id.replace(
				"filters_autocomplete_row_elem_",
				""
			);
			...
			for (
				var i = 0;
				i < existing_tokens[dropdown_class_name].length;
				i++
			) {
				if (
					existing_tokens[dropdown_class_name][i]["name"] ==
					input_token
				) {
					j++;
				}
			...
			if (j != 0) {
				console.log(
					"Existing token exists. not adding more tokens.:::",
					dropdown_class_name
				);
			} else {
				existing_tokens[dropdown_class_name].push(
					new UtilityFunctions().takeInputValueAndReturnObject(
						input_token
					)
				);
			...
			if (dropdown_class_name == "company_name") {
				...
				company_id = user_input
					.getElementsByClassName("alpha company-name")[0]
				...
				console.log(
					"Length == ",
					existing_tokens["company_name"].length
				);
	...
	return <div>ParamsStorage</div>;
};

export default ParamsStorage;
