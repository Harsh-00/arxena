	...
	useEffect(() => {
		...
		class SessionStorageUpdates {
			updatePromptEnumTextareaList() {
				sessionStorage["prompt_enum_list"] = JSON.stringify(
					$("#prompt_enum_textarea").textext()[0].tags()["_formData"]
				);
			}
		...
		class Pages {
			refreshUserfromBackend() {
				...
				$.ajax({
					...
					var payment_plans = JSON.parse(
						sessionStorage["payment_plans"]
					);
					var geo = sessionStorage["geo_country_selected"];
					var paymentPlansGeoObj =
						new PaymentPlans().getPaymentPlansBasedonGeo(geo);

					var payment_geo_selected =
						sessionStorage["geo_country_selected"];
					...
					if (typeof data != "string") {
						...
						if (plan_data["plan"] == "plan_0") {
							...
							var date_next_renewal =
								plan_data["date_next_renewal"];
							...
							var credits_topup_section = document
								...
								.replaceAll(
									"{credits_2}",
									paymentPlansGeoObj["credits_2_credits"]
								)
								.replaceAll(
									"{credits_3}",
									paymentPlansGeoObj["credits_3_credits"]
								)
								.replaceAll(
									"{credits_4}",
									paymentPlansGeoObj["credits_4_credits"]
								)
								.replaceAll(
									"{credits_5}",
									paymentPlansGeoObj["credits_5_credits"]
								)
								.replaceAll(
									"{credits_1_price}",
						...
						try {
							var num_people_referred =
								plan_data["people_referred"].length;
						} catch {
						...
						document.getElementById("all_content").innerHTML =
							document
								...
								.replace(
									/{current_plan_label}/g,
									current_plan_label
								)
								.replace(
									/{google_auth_status}/g,
									google_auth_status
								)
								.replace(
									/{date_next_renewal}/g,
								...
								.replace(
									/{total_credits_avalable}/g,
									total_credits_avalable
								)
								.replace(
									/{num_people_referred}/g,
									num_people_referred
								)
								.replace(
									/{num_people_referred_label}/g,
									num_people_referred_label
								)
								// .replace(/{api_key_section}/g, api_key_section)
								.replace(
									/{credits_topup_section}/g,
									credits_topup_section
								);

						document.getElementById(
							"pricing_payment_section"
						).innerHTML = document.getElementById(
							"pricing_payment_data"
						).innerHTML;
			...
			pricingPage() {
				...
				document.getElementById(
					"pricing_page_pricing_payment_section"
				).innerHTML = document.getElementById(
					"pricing_payment_data"
				).innerHTML;
				new PaymentPlans().setDashboardPrices();
			...
			jobsPage() {
				...
				var height_for_onboarding_flow = (
					$(window).height() -
					122 -
					6
				).toString();
				...
				let customer_status = JSON.parse(localStorage["user"])["plan"][
					"customer_status"
				];
				if (customer_status != "unpaid_customer") {
					document.getElementById(
						"inside_panel_content_container"
					).innerHTML = document.getElementById(
						"inside_panel_content_account_element"
					).innerHTML;
					document.getElementById(
						"inside_panel_content_container_mobi"
					).innerHTML = document.getElementById(
						"inside_panel_content_account_element"
					).innerHTML;
					...
					document.getElementById("header_element").innerHTML =
						document.getElementById(
							"org_chart_header_element"
						).innerHTML;
					new ListPageElements().createAllJobsListings();
					document.getElementById(
						"left_panel_toggle_navigation_companies"
					).style.fontWeight = "";
					document.getElementById(
						"left_panel_toggle_navigation_companies"
					...
					document.getElementById(
						"left_panel_toggle_navigation_people"
					).style.fontWeight = "";
					document.getElementById(
						"left_panel_toggle_navigation_people"
					).style.textDecoration = "";
					...
					document.getElementById(
						"left_panel_toggle_navigation_orgcharts"
					).style.fontWeight = "";
					document.getElementById(
						"left_panel_toggle_navigation_orgcharts"
					).style.textDecoration = "";
					...
					document.getElementById(
						"left_panel_toggle_navigation_lists"
					...
					document.getElementById(
						...
						document.getElementById("payment_page").innerHTML;
				}
			}
			...
			setLoginLogoutTopBar() {
				if ("email" in localStorage) {
					if (sessionStorage["logon"] == "true") {
						...
						document.getElementById(
							"elements_loggedin_logged_out_mobi"
						).innerHTML = document
			...
			setAllContentLoginLogout() {
				...
				document.getElementById("all_content").innerHTML = document
					...
					.replace(
						/{height_for_onboarding_flow}/g,
						height_for_onboarding_flow
					);
				...
				document.getElementById(
					"toggle_navigation_companies_people"
				).innerHTML = document.getElementById(
			...
			tablesPage() {
				...
				try {
					var table_name = typeof JSON.parse(
				...
				if (table_name === "undefined") {
					...
					document.getElementById(
						...
						"table_section_center"
					).innerHTML;

					document.getElementById("left_search_filters").innerHTML =
						document
							.getElementById("left_panel_tables_list")
							.innerHTML.replaceAll(
								"{table_id}",
								JSON.parse(sessionStorage["current_table"])[
									"table_id"
								]
							)
							.replaceAll(
					...
					document.getElementById(
						...
						"right_panel_sublists_section"
					).innerHTML;
					document.getElementById("header_element").innerHTML =
						document.getElementById(
							"header_element_lists"
						).innerHTML;
		...
		class PageContents {
			async setTopBar() {
				var extension_status =
					await new CheckChromeExtension().getExtensionStatus();
				// extension_status = false;
				console.log("Extension status::", extension_status);
				if (!extension_status) {
					document.getElementById("topbar_advert").style.display =
						"table";
				} else {
					document.getElementById("topbar_advert").style.display =
						"none";
			...
			clearHeaderElement() {
				...
				try {
					...
					document.getElementById("linkedin_icon_header").innerHTML =
						"";
					document.getElementById(
						"functional_dropdown_container"
					).innerHTML = "";
					document.getElementById(
						"geo_dropdown_container"
					).innerHTML = "";
				} catch (error) {
					console.log("Error:", error);
				}
			...
			logIn() {
				...
				if (
					...
					console.log(
						"email and password are in somewhat in the session "
					);
					document.getElementById("login_error").innerHTML =
			...
			signUp() {
				...
				if (
					...
					var session_storage_visitor_fp =
						sessionStorage["visitor_fp"];
							...
							if (data.status == "user_created") {
								...
								if (data["status"] == "visitor_fp_exists") {
									...
									if (
										...
										data["status"] ==
										"user_already_registered_with_phone"
									) {
										document.getElementById(
											"signup_error"
										).innerHTML =
			...
			changePage(extension) {
				...
				console.log(
					"This is the localstorage USER ID::",
					localStorage["user_id"]
				);
				...
				try {
					var pages_list =
						sessionStorage["navigation_history"].split(",");
				...
				if (new_page == "/login") {
					...
					console.log(
						"Got here with new page is equal to org chart page"
					);
					console.log(sessionStorage["query"]);
					// if (typeof(JSON.parse(sessionStorage['query'])['company_id']['name']) != "undefined"){
					if (
						(typeof sessionStorage["query"] != "undefined") &
						(typeof JSON.parse(sessionStorage["query"])[
							"company_id"
						]["name"] !=
							"undefined")
					) {
						...
						window.history.pushState(
							...
							extension +
								"/" +
								JSON.parse(sessionStorage["query"])[
									"company_id"
								]["name"]
						);
		...
		class ProcessNodesStates {
			resetSessionStorageforDropdowns() {
				...
				sessionStorage["query"] =
					sessionStorage["blank_session_storage_obj"];
		...
		function callBodyOnloadFunction() {
			console.log(
				"callBodyOnloadFunction function has been called in javascript"
			);
			load_type = document
				.getElementById("load_type")
				.getAttribute("data-row");
			user_obj = document
				.getElementById("user_obj")
				.getAttribute("data-row");
			...
			if (load_type == "org-chart") {
				...
				if (window.location.href.indexOf("/org-chart/") > 0) {
					...
					console.log(
						"Load type is org-chart. Going to draw org charts!!!"
					);
					...
					if (
						parsed_flask_jinja_obj["query_status"] == "unauthorized"
					) {
						console.log("Some problem");
					} else {
						var org_data =
							parsed_flask_jinja_obj["results"]["data"][
								"modified_query_results"
							];
						console.log(org_data);
						if (org_data.length == 0) {
							new Query().handleIncompleteResponse(
								parsed_flask_jinja_obj
							);
						}
						// Checking if this is needed and will worl
						if (
							window.location.href.split("/org-chart/")[1]
								.length > 0
						) {
		...
		class Query {
			showWaitForPeopleDataFromExternalSource() {
				console.log(
					"Got to show wait for people from external sources"
				);
			...
			setUpCountQueryResults() {
				...
				if (sessionStorage["data_source"] != "data_broker_1") {
					if (sessionStorage["current_focus"] != "orgcharts") {
						console.log(
							"Calling callBackendQueryExecuteOnResponse for count"
						);
				...
				jQuery
					.ajax({
						...
						error: function (xhr, status) {
							...
							if (error_status == 500) {
								new ProcessNodesStates().resetSessionStorageforDropdowns();
								console.log(
									"There has been an error as error code is 500!!"
								);
								// location.reload();
					...
					.done(function (response, statusText, xhr) {
						...
						if (
							...
							console.log(
								"Going here to set timeout and change page to /login"
							);
							setTimeout(
								"new PageContents().changePage('/login')",
								300
			...
			executeQueryResponse(response, statusText, xhr, user_input) {
				...
				if (
					...
					if (response["status"] == "success") {
						...
						new LoadDataPeopleCompaniesOrgCharts().loadResults(
							response
						);
						$(waiting_results_raw_string).modal("hide");
					...
					console.log(
						"Called executeQueryResponse from cost_credits ::"
					);
					...
					if (
						...
						if (response["query_status"] == "unauthorized") {
							console.log(
								"GOT RESPONSE WITH status is unauthorized ,incomplete"
							);
					...
					if (
						...
						"fetchCurrentNodeDataResults"
					) {
						console.log(
							"GOT RESPONSE WITH fetchCurrentNodeDataResults"
						);
						console.log(response);
						...
						updateStorageToken(
							{},
							"resetOrgChartTokensAfterFetchNodeData"
						);
					} else if (
						response["selected_context_menu"] ==
						"fetchEntireCompanyDataResults"
					) {
						console.log(
							"GOT RESPONSE WITH fetchEntireCompanyDataResults"
						);
						console.log(response);
						new LoadDataPeopleCompaniesOrgCharts().loadResults(
							response
						...
						response["selected_context_menu"] ==
						"fetchAllLeadershipNodes"
					) {
						console.log(
							"GOT RESPONSE WITH fetchAllLeadershipNodes"
						);
						console.log(response);
						var response_nodes =
							response["results"]["data"][
						...
						response["selected_context_menu"] ==
						"fetchAllSelectedNodes"
					) {
						console.log("GOT RESPONSE WITH fetchAllSelectedNodes");
						console.log(response);
					} else if (
						response["selected_context_menu"] ==
						"fetchAllFunctionDataResults"
					) {
						console.log(
							"GOT RESPONSE WITH fetchAllFunctionDataResults"
						);
						console.log(response);
						console.log(
							"GOT RESPONSE WITH fetchEntireCompanyDataResults"
						...
						changeNodeDataUsingQueryResponse(
							response,
							node_key,
							parent
						);
			...
			handleIncompleteResponse(response) {
				...
				console.log("This is the client query obj::");
				console.log(response["request_id"]);
				var company_employee_count_obj = JSON.parse(
				...
				if (response["query_status"] == "no_credits") {
					document.getElementById(
						...
						.innerHTML.replace(
							"{no_results_message}",
							"<br>Apologies but you need to recharge your credits for further searches. Please recharge from the dashboard or email support at data@arxena.com and we will help immediately.</a>"
						);
					document.getElementById(
						"right_search_results_people_companies_orgcharts_mobi"
					).innerHTML = document
					...
					if (company_employee_count_obj === undefined) {
						if (sessionStorage["current_focus"] == "orgcharts") {
							var company_name_requested = window.location.href
								.split("/")
								.slice(-1)[0];
							document.getElementById(
								"right_search_results_people_companies_orgcharts"
						...
						company_name_requested =
							company_employee_count_obj["company_id_label"];
						document.getElementById(
							"right_search_results_people_companies_orgcharts"
						).innerHTML = document
							.getElementById("no_results_message")
			...
			fetchQueryShowResults(user_input) {
				console.log(
					"Called fetchQueryShowResults with the user input:::",
					user_input,
					"and data source after fetchQueryShowResults",
					sessionStorage["data_source"]
				);
				var loader2 =
					'<div style="width:100%;text-align:center; padding:40px"><img style="width:30px" src="/static/img/new_loading2.gif"/></div>';
				if (
					user_input === undefined ||
					user_input == "load_data_without_loading_tokens"
				) {
					console.log(
						"This the user input and it is undefined ::",
						user_input
					);
					...
					if (
						...
						console.log(
							"Tokens list is not equal to blank session Object"
						);
						console.log(
						...
						if (sessionStorage["load_type"] != "org-chart") {
							...
							if (
								...
								new DrawOrgCharts().setupOrgChartItems(
									parsed_flask_jinja_obj
								);
							} else {
								new Query().callBackendQueryExecuteOnResponse(
					...
					user_input == "fetch_people_results_consume_credits"
				) {
					console.log(
					...
					if (
						...
						console.log(
							"Session Storage is blank or company ID is undefined"
						);
					}
				} else if (
					...
					if (user_input == "locations_dropdown1") {
						document.getElementById(
							"modal_locations_dropdown1"
						).innerHTML = loader2;
					} else if (user_input == "locations_dropdown2") {
						document.getElementById(
							"modal_locations_dropdown2"
						).innerHTML = loader2;
					} else if (user_input == "locations_dropdown3") {
						document.getElementById(
							"modal_locations_dropdown3"
						).innerHTML = loader2;
					}
					if (
						sessionStorage[
							"selectFunctionsTreeFunctionsRootItem"
						] != "" ||
						...
						updateStorageToken(
							node,
							"fetch_query_locations_dropdown"
						);
					...
					if (user_input == "locations_dropdown2") {
						var country_selected =
							sessionStorage["locations_dropdown1"];
						var regions_arr =
							new DrawOrgCharts().getRegionsFromCountry(
								country_selected
							);
					...
					if (sessionStorage["current_focus"] == "orgcharts") {
						...
						if (
							...
							sessionStorage[
								"selectFunctionsTreeFunctionsItem"
							] != "" ||
							sessionStorage["selectGradesItem"] != ""
						) {
							console.log(
								"This is the selcted using the main functions root"
							);
						} else {
							try {
								if (
									typeof sessionStorage["selectedNodeKey"] !=
									"undefined"
								) {
									var node = myDiagram.findNodeForKey(
										sessionStorage["selectedNodeKey"]
									...
									if (
										node.data.std_function_category !=
										"ceoassist"
									) {
						...
						document.getElementById(
							"cost_of_credits_people"
						).innerHTML = loader2;
					} else if (sessionStorage["current_focus"] == "companies") {
						document.getElementById(
							"cost_of_credits_companies"
						).innerHTML = loader2;
					}
		...
		class UtilityFunctions {
			...
			runNavigation() {
				if (window.location.href.indexOf("/signup") >= 0) {
					...
					window.location.href.indexOf("/password_reset_link/") >= 0
				) {
					new PageContents().setLogonStatus();
					// this is a referral link
					var metadata = window.location.href.slice(
						window.location.href.indexOf("/password_reset_link/") +
							21,
						window.location.href.length
					);
					...
					window.location.href.indexOf("/password_reset") >= 0
				) {
					...
					console.log(
						"Cant allocate any specific URL to page. Going to home page"
					);
					sessionStorage["navigation_history"] = "";
					new PageContents().setLogonStatus();
					new PageContents().changePage("/#");
			...
			setUPSessionStorageVariables() {
				...
				sessionStorage["raw_string_query_input"] = "";
				sessionStorage["load_type"] = document
					.getElementById("load_type")
					.getAttribute("data-row");
				// sessionStorage['nodes_accessed'] = '[]'
				sessionStorage["upload_type_jd_cv"] = "";
				sessionStorage["payment_plans"] = document
					.getElementById("payment_plans")
					.getAttribute("data-row");
				sessionStorage["current_focus"] = "orgcharts";
				...
				try {
					sessionStorage["use_case"] = JSON.parse(
						document
							.getElementById("user_obj")
							.getAttribute("data-row")
					)["use_case"];
				...
				sessionStorage["raw_text_filters"] =
					JSON.stringify(raw_text_filters);
				sessionStorage["sample_people_data"] = "true";
				sessionStorage["sample_company_data"] = "true";
			...
			detectModalClosuresAndCloseItems() {
				$(fetch_node_results).on("hide.bs.modal", function () {
					...
					document.getElementById(
						"select_functions_tree_data_dropdown"
					).style.display = "none";
					document.getElementById(
						"select_grades_data_dropdown"
					).style.display = "none";
					sessionStorage["selectFunctionsTreeFunctionsRootItem"] = "";
					sessionStorage["selectFunctionsTreeFunctionsItem"] = "";
					sessionStorage["selectGradesItem"] = "";
				...
				$(import_data_modal).on("hide.bs.modal", function () {
					sessionStorage["csv_excel_data"] = "";
					document.getElementById(
						"column_mapping_interface"
					).style.display = "none";
					$("#filename").val("");
					document.getElementById("file_name_uploaded").innerText =
						"";
					document.getElementById(
						"completed_upload_file_submission"
					).innerText = "";
				});
				...
				$(upload_jdcv_modal).on("hide.bs.modal", function () {
					...
					document.getElementById(
						"pick_upload_jd_cv_error"
					).innerHTML = "";
					document.getElementById(
						"upload_jd_cv_dropdown_selected"
					).innerHTML = "Please select upload type - JD/ CV";
				});
				...
				$(upload_jd_modal).on("hide.bs.modal", function () {
					document.getElementById(
						"upload_jd_modal_extra_info"
					).innerHTML = "";
					document.getElementById(
						"upload_new_jd_form_err"
					).innerHTML = "";
			...
			stopLoadersFromRotating() {
				for (
					...
					document.getElementsByClassName("loader-link")[
						i
					].innerHTML = "";
				}
			}
			...
			pageBack() {
				...
				var previous_pages_list =
					sessionStorage["navigation_history"].split(",");
				console.log(previous_pages_list);
				var previous_page =
					previous_pages_list[previous_pages_list.length - 2];
			...
			parseExcel(excelData) {
				...
				if (workbook.SheetNames.includes("Pipeline Submissions")) {
					...
					var pipelineSubmissionsSheet =
						workbook.Sheets["Pipeline Submissions"];
					...
					var workExperienceSheet =
						workbook.Sheets["Work Experience"];
					var workExperienceData = XLSX.utils.sheet_to_json(
						workExperienceSheet,
						{
							header: 1,
						}
					...
					for (var i = 0; i < pipelineSubmissionsData.length; i++) {
						...
						var firstWorkExperience = workExperienceData.find(
							function (workExperience) {
								return (
									workExperience[1] === pipelineSubmission[1]
								);
							}
						);
			...
			scrollToPart() {
				console.log(
					"Trying to scroll to part where ceo is grade and function"
				);
				...
				if (center_key_node != undefined) {
					...
					try {
						myDiagram.centerRect(
							myDiagram.findNodeForKey(10000).actualBounds
						);
					} catch {}
				} else {
					myDiagram.commandHandler.scrollToPart(center_key_node);
			...
			openHelpModal() {
				...
				$("#load_org_chart_helper").toggleClass(
					"account_row account_row-selected"
				);
			}
			...
			get_gpt_cost_by_token(chipText = "") {
				let output_option = $("#prompt_enum_textarea")
					.textext()[0]
					.tags()["_formData"];
				let question_prompt = document.getElementById(
					"input_prompt_base_filter_text"
				).value;
				let selectDropDownColumn =
					new Components().getAllSelectedFields(
						"selected_option_chips"
					);
				let gpt_prompts = [
					{
						type: "defined",
				...
				let columns = gpt_prompts
					.filter((prompt) =>
						selectDropDownColumn.includes(prompt.field)
					)
					.map((matchedPrompt) => matchedPrompt.data);
				columns = ["_id", ...columns];
				let hot = hotInstance.getHotInstance();
				let candidate_details =
					new LeftPanelTableActions().getSelectedCandidateDetailsFromHOT(
						hot,
						columns
					);
				console.log(
					"These are the candidate details::",
					candidate_details
				);
				candidate_details = candidate_details
					.filter((detail) => detail != null && detail._id != null)
					.map(({ _id, ...rest }) => rest);
				console.log(candidate_details);
				if (!!candidate_details && candidate_details?.length > 0) {
					let valuesArray = candidate_details
					...
					$.ajax({
						...
						if (data["status"] == "ok") {
							...
							document.getElementById(
								"gpt_cost_compute_element"
							).innerHTML = `The approx cost for ${total_tokens} tokens is $${cost} and model used ${GPT_model}`;
						}
					});
				} else {
					console.error(
						"no records selected in get_gpt_cost_by_token"
					);
				}
			}
